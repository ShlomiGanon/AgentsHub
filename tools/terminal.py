"""Shared building blocks for `tools/terminal_client_commander.py` and `tools/terminal_client_viewer.py` — not a governed entry point (same status `tools/terminal_client.py` itself..."""
...
from bot.client import HttpApiClient, TelegramClient, _do_request
...
def ensure_bot_service_commander(profile_module_path: str, profile_module) -> None:
    ...
    rc = _run_user_admin(["--profile", profile_module_path, "add", "--telegram-id", BOT_SERVICE_IDENTITY, "--level", "commander"])
    if rc != 0:
        raise SystemExit(
...
def ensure_test_identity(profile_module_path: str, test_identity: str, level: str, profile_module) -> bool:
    ...
    rc = _run_user_admin(["--profile", profile_module_path, "add", "--telegram-id", test_identity, "--level", level])
    if rc != 0:
        raise SystemExit(
...
def cleanup_test_identity(profile_module_path: str, test_identity: str, created_this_session: bool, cursor_path: Path | None = None) -> None:
    ...
    rc = _run_user_admin(["--profile", profile_module_path, "remove", "--telegram-id", test_identity])
    if rc != 0:
        print(
