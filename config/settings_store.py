...
Holds exactly three live values: the retry attempt count, the risk
threshold, and the precedent lookback window. Everything else needs a
restart. On first start of a deployment the three starting values come
from the profile; on every later start the JSON file beside the database
is preferred, so an operator's mid-incident change survives a restart.
"""
...
class SettingsStore:
    ...
    def _write(self) -> None:
        # Write to a temp file and replace, so a crash mid-write can never
        # leave a truncated settings file behind — the same safety pattern
        # §4.3 uses for profile edits.
