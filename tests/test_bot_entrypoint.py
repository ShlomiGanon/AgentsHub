...
def test_unregistered_sender_is_refused_before_anything_is_submitted():
    ...
    reply = _run(handle_incoming_message(_deps(api), "stranger", "there is a fire"))
...
def test_question_returns_the_answer_directly():
    ...
    reply = _run(handle_incoming_message(_deps(api), "v1", "how many events last week?"))
...
def test_report_acknowledges_with_job_id_and_kind():
    ...
    reply = _run(handle_incoming_message(_deps(api), "v1", "there is smoke near the depot"))
...
def test_request_awaiting_approval_says_so():
    ...
    reply = _run(handle_incoming_message(_deps(api), "c1", "dispatch a response"))
