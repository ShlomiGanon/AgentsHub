...
def test_control_channel_accepts_only_discovered_profiles(tmp_path, monkeypatch):
    ...
    command_id = server_control.submit_command("switch_profile", profile_module="profiles.response_team")
    command = server_control.consume_command()
    assert command["id"] == command_id
    assert command["profile_module"] == "profiles.response_team"
    for unavailable_profile in ("profiles.standby_squad", "profiles.fire_station", "os"):
...
def test_failed_profile_switch_rolls_back_and_keeps_error_for_admin(monkeypatch):
    ...
    assert starts == ["profiles.firefighting", "profiles.response_team"]
    assert supervisor.profile_module == "profiles.response_team"
    assert "restored profiles.response_team" in supervisor.last_error
