"""Consolidated responsibility module for runtime."""
...
from bot.presentation import format_failure_notice, format_header, format_job_result
...
async def dispatch_notification(deps: "BotDeps", notification: "BotNotification") -> None:
    if notification.kind == "clarification_hold":
        await presentation.push_clarification_prompt(deps, notification.payload)
        return

    if notification.kind == "approval_hold":
        await presentation.push_approval_prompt(deps, notification.payload)
        return

    if notification.kind == "uncertain_verdict":
        await presentation.notify_uncertain_verdict(deps, notification.payload)
        return
    ...
    if notification.kind == "no_match_notice":
        await presentation.notify_no_match(deps, notification.payload)
        return
