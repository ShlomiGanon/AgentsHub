...
| `agents` | `agents.registry`, `agents.results`, `agents.errors`, `agents.reference`, `agents.history` | Concrete agent modules are public because profiles construct them directly. `agents.base`, `agents.descriptor`, `agents.tooling`, and `agents.adapter` stay internal. |
| `protocols` | `protocols.model`, `protocols.loader` *(lands in §4)* | |
| `history` | `history.interface`, `history.query` | `history.interface` exposes writes, extraction, and scheduler hooks; `history.query` exposes historical Q&A and precedent lookup. Other history modules are internal. |
...
| `agents` | `agents.registry`, `agents.results`, `agents.errors`, `agents.reference`, `agents.base` | `agents.reference` is the reference agent (§3.11) — a concrete agent module a profile constructs directly, not a query interface. Every future concrete agent (a real domain specialist) becomes an entry point the same way, one module per agent. `agents.base` became a real entry point in Mission 6: the work plan's own branch table places core agents like `orchestrator.main_agent.MainAgent` *outside* `agents/`, and a concrete agent must genuinely subclass `agents.base.Agent` at runtime, not just reference it as a type hint — so any package defining a concrete agent needs it. `agents.descriptor`, `agents.tooling`, `agents.adapter` stay internal — only agents/ package files import them. |
