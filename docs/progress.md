...
## 2026-09-24 - docs/bar_improves.md Stage 1: tolerate a malformed OPTIONAL extracted field

- Confirmed the actual current behavior first: `history/event_pipeline.py`'s `extract_event` ran every optional scalar field (`classification`, `area`, `description`, `severity`, `occurred_at`) through `_optional_string`, which raised `ExtractionExecutionError` — rejecting the *entire* report — the moment the model returned a non-string, non-null value (an object or a list). `entities` had the identical failure mode for a non-list-of-strings value. Required-ness (which fields matter for a given event type) is a separate, downstream concern (`profiles.contracts.EventTypeRegistry.required_fields_for`, checked later by `orchestrator.flows._apply_required_fields_gate`) that `extract_event` itself has no concept of — every field it parses is optional in `docs/vocabulary.md`'s sense.
- `_optional_string` (and the inline `entities` check) now normalize a wrong-type value to `None`/`()` instead of raising, and log one structured INFO record (`extraction_optional_field_dropped`, naming only the field and `type(value).__name__` — never the value itself) through the existing `logging`/`tools.get_trace_id` machinery. The out-of-registry classification/area check (a correctly-typed string just absent from the profile's list) and the ISO-8601 format check on a correctly-typed but malformed `occurred_at` string are both untouched — only the "wrong Python type" failure mode changed.
- As part of the same read-through, tightened the generic (no domain terms) `entities` extraction instruction in `_prompt()` to "list every identifier the report refers to, e.g. equipment or unit identifiers" — this is also Stage 4c's own requirement, implemented here since it's the same prompt text.
- Added `test_malformed_optional_field_is_dropped_not_the_whole_report`, `test_malformed_optional_entities_field_is_dropped_not_the_whole_report`, `test_valid_optional_fields_are_unaffected_by_the_malformed_field_tolerance` to `tests/test_history_event_pipeline.py`. The pre-existing `test_invalid_closed_set_values_are_left_unresolved` (out-of-registry) and `test_code_fence_is_the_only_cleanup_and_bad_json_is_an_execution_error` (bad JSON still rejects) both still pass unchanged, confirming the two explicitly-preserved behaviors.
- Focused verification: 36 history-event-pipeline tests passed.

## 2026-09-24 - docs/bar_improves.md Stage 2: one retry of extraction on a model timeout or model error

- Confirmed the raw report text is already durably persisted before extraction ever runs (`orchestrator.flows.begin_report` → `record_initial_event`, synchronous, no model call) — so a transient provider failure during extraction loses no data on its own; the gap was that `extract_event`'s own `except Exception` around the `model_invoker(prompt)` call converted *any* exception, including `agents.AgentTimeoutError`/`AgentModelError` propagating uncaught out of `MainAgent.process()`, straight into a terminal `ExtractionExecutionError` → `"failed"` outcome after exactly one attempt.
- Implemented the retry in `orchestrator/flows.py`'s `_model_invoker_for` (the callable `extract_event` invokes), not in `history/event_pipeline.py`: `history` has no existing dependency on `agents`' error types (`docs/allowed_calls.md`'s package layering keeps history and agents as peers under orchestration), and `orchestrator.flows` already imports both, so this keeps `history` agent-agnostic — documented as a deviation below. Catches `AgentTimeoutError`/`AgentModelError` from the first `main_agent.process(prompt, [])` call, logs a `extraction_retry` INFO record (`cause`, `trace_id`, matching the existing `step_retry` logging style in `protocols/executor.py`), and makes exactly one further attempt; a failure on that second attempt propagates normally into `extract_event`'s existing `except Exception` → `ExtractionExecutionError` → `"failed"` path, unchanged. A parse-type failure (bad JSON, Stage 1's tolerated malformed field) is not retried — it never raises `AgentTimeoutError`/`AgentModelError` in the first place. `MODEL_TIMEOUT_SECONDS` and the zero provider-retry configuration are untouched; this is one additional application-level attempt, not a provider retry.
- Added `test_extraction_retries_once_on_a_model_timeout_then_succeeds`, `test_extraction_retries_once_on_a_model_error_then_succeeds`, `test_extraction_fails_after_two_timeouts_never_attempts_a_third_time` to `tests/test_orchestrator_flows.py`, using a small duck-typed agent stand-in (`_TimeoutThenScriptedAgent`) that fails its extraction call a fixed number of times before behaving like the existing `_ScriptedAgent`.
- Focused verification: 65 orchestrator-flow tests passed (full file).

## 2026-09-24 - docs/bar_improves.md Stage 3: availability fields for absence reports

- Added `availability_start`, `availability_end`, `absence_reason` to `protocols.EVENT_DATA_FIELDS`. Everything downstream of that one tuple — `profiles.loader.validate_profile`'s `EVENT_TYPE_REQUIRED_FIELDS` field-name validation, `orchestrator.reasoning.formulate_event_data_question`/`extract_event_data_update` (the event-data-hold question and reply-parsing prompts), and the pre-formulation required-fields gate itself (`orchestrator.flows._apply_required_fields_gate`, which reads columns off the persisted event row generically by name) — is already written generically over `EVENT_DATA_FIELDS`/`event.get(name)` and needed **no** changes to support the new fields; this is what "behavior comes entirely from the existing gate" meant in practice. No new hold kind was added; these are ordinary `event_data` holds, not clarification holds.
- New idempotent migration 20 (`persistence/schema.py`; the log's highest was confirmed to be 19) adds three nullable `TEXT` columns to `events`, guarded the same way migrations 17/18/19 already are (inspect `PRAGMA table_info(events)` first; also tolerates the table not existing yet, needed for two pre-existing synthetic single-table migration fixtures in `tests/test_migrations.py`). The fresh-schema `EVENTS_TABLE_DDL` gained the same three columns in the same change. `persistence/sqlite_store.py`'s `_EVENT_COLUMNS` (insert/select) was extended to match; the columns are plain `TEXT`, not JSON/bool-encoded, and are not in `_EVENT_IMMUTABLE_COLUMNS`, so they're updatable through the existing generic `update_event` path with no further change.
- `history/field_catalog.py` gained three narrative `EventFieldDefinition` entries (English label + meaning). `history/contracts.py`'s `ExtractionResult` gained the three fields (keyword-defaulted, so every existing positional construction of it, in production and tests, is unaffected). `history/event_pipeline.py`'s extraction prompt now asks for the three keys explicitly (with an instruction never to guess an interval); `extract_event` parses them through the same tolerant, Stage-1-style path (wrong type or unparseable timestamp → dropped and logged, not rejected) and normalizes a parseable timestamp to the existing `storage_timestamp` format before returning it — a deliberate choice, since occurred_at itself is *not* normalized at extraction time in the pre-existing code (only when a later reply updates it), but the two new fields have no pre-existing callers/tests to preserve and "timestamps use the existing storage timestamp format" reads most naturally as applying at write time. `record_extracted_fields` persists all three; `EVENT_DATA_UPDATE_FIELDS` (the reporter-reply update allowlist) was extended to include them too.
- `orchestrator/flows.py`'s `apply_event_data_reply` (the existing reporter-reply path) now normalizes a supplied `availability_start`/`availability_end` to storage-timestamp format the same way it already does for `occurred_at`, and rejects (raises the same `OrchestrationParseError` the existing invalid-`occurred_at` case already raises) an `availability_end` before its own `availability_start` — comparing whichever value is newly supplied or already on the event. This reuses the exact existing "invalid reply" mechanism end to end: `api/routes.py` already abandons a hold and re-routes the message as a new one on `OrchestrationParseError`, so no new branch was needed for the rejection case.
- Noted, not changed (pre-existing, out of this stage's scope): `apply_event_data_reply`/`resume_after_event_data` resolve and resume an `event_data` hold as soon as *any* requested field is filled by a reply, without re-checking that every declared required field is now present before continuing to risk assessment. This only matters if a single reply supplies one of two required fields and a later reply is needed for the other — the scenario this stage actually specifies (a single reply giving the whole interval at once) is unaffected.
- Added migration tests (`tests/test_migrations.py`: existing-database upgrade, fresh database, idempotent rerun) and flow-level tests (`tests/test_orchestrator_flows.py`: interval-given-up-front proceeds with no hold; interval-omitted holds for exactly `availability_start`/`availability_end` with `absence_reason` stored; a follow-up reply fills both fields and the event resumes to a normal outcome; `availability_end` before `availability_start` is rejected).
- Focused verification: 3 new migration tests + 4 new flow tests passed; full `tests/test_orchestrator_flows.py`, `tests/test_history_event_pipeline.py`, `tests/test_migrations.py`, `tests/test_orchestrator_holds.py`, `tests/test_orchestrator_reasoning.py`, `tests/test_persistence_conformance.py`, `tests/test_persistence_events.py`, `tests/test_sqlite_store.py`, `tests/test_profile_loading.py`, `tests/test_protocol_repository.py`, `tests/test_question_answering.py`, `tests/test_orchestrator_insights.py`, `tests/test_orchestrator_judgment.py`, `tests/test_orchestrator_selection.py`, `tests/test_history_query.py`, `tests/test_history_agent.py` (456 tests) passed. Full offline suite after Stages 1-3: 1485 passed, 2 failed — `tests/test_file_catalog.py` (pre-existing gap, `docs/bar_improves.md` itself untracked and not yet catalogued; will be closed once this task's new files are added to the catalog) and `tests/test_integration_user_administration.py::test_user_api_exposes_reads_self_name_update_and_commander_approval` (confirmed pre-existing and unrelated — no file this task has touched so far is anywhere near the admin-panel user routes it asserts on).

## 2026-09-24 - docs/bar_improves.md Stage 4: the two operational profiles (SEC/FIRE)

- Confirmed first that `docs/bar_improves.md`'s own background references (`profiles/demo.py`, `agents/reference.py`) no longer exist on disk — commit `0376967` ("remove demo profile") already replaced that pattern with `profiles/standby_squad.py`/`profiles/firefighting.py` (an operationally similar SEC/FIRE pair, but a heavier one with its own SQLite-backed surveillance/team-status side-stores) and `agents/standard_agents.py::ReferenceAgent`. Followed the actual current profile-authoring convention instead: `profiles/template.py` and `profiles/standby_squad.py`/`profiles/firefighting.py`'s own shared-agent pattern (`agents/friendly_forces_agent.py` used unmodified by two profiles, a profile-specific subclass binding only deployment-specific class attributes).
- New shared agent modules, following `agents/friendly_forces_agent.py`'s existing pattern of "reusable across profiles, no external system integration, a tool records what it did and returns a precise text result stating only the tool's own effect": `agents/roster_agent.py` (`RosterAgent`, `record_availability` — shared unchanged by both profiles, exactly like `FriendlyForcesAgent` already is), `agents/response_team_agents.py` (`SecurityOpsAgent`: `log_observation`/`notify_team`/`request_friendly_force_dispatch`/`recall_drone`; `SurveillanceFaultAgent`, registry name `surveillance_agent`: `log_camera_fault`/`request_technician`), `agents/fire_station_agents.py` (`DispatchAgent`: `dispatch_station_crew`/`request_mutual_aid`; `HazmatAgent`: `request_hazmat_assessment`). All five registered in `agents/__init__.py`'s public facade (`__all__`), matching how `FriendlyForcesAgent`/`SurveillanceAgent`/`TeamStatusAgent` are already exposed.
- **Deviation from the literal spec text:** Stage 4b's "roster_agent — same as SEC (define it in the FIRE profile too)" reads, taken completely literally, as duplicating the class. Read instead as "do not import one *profile module* from the other" (the sentence's own next clause, and the actual concern hard constraint 2 protects against) — not as "do not share a reusable agent class via the `agents` package," which is exactly the existing `FriendlyForcesAgent` pattern this codebase already uses for two profiles. Defined `RosterAgent` once, shared, for consistency with existing code and to avoid a needless duplicate-maintenance class.
- **Deviation:** `MUTUAL_AID_RESOURCES = ("ASHED", "CARMEL")` is declared in `profiles/fire_station.py` exactly as specified (a profile-level constant, no capability invented for the names), and bound onto a profile-specific `FireStationDispatchAgent(DispatchAgent)` subclass via a class attribute (`DispatchAgent.MUTUAL_AID_RESOURCES`, empty by default) — the same binding pattern `profiles/standby_squad.py` already uses for `surveillance_db_path` — rather than hardcoding the resource names inside the shared, reusable `agents/fire_station_agents.py` module, which must stay free of any one deployment's specific resource names.
- `profiles/response_team.py` (port 8907) and `profiles/fire_station.py` (port 8908) declare every attribute `docs/profile_spec.md` requires, `DEFAULT_LANGUAGE = "he"` exactly as the spec's own attribute list shows (this only selects which of the two already-existing, already-validated `messages/en.py`/`messages/he.py` catalogs supplies this deployment's *fixed* API/Telegram/CLI boilerplate text — every word this task actually wrote, in both new profile files and both new agent files, is English, satisfying the Hebrew-leakage hard constraint, confirmed by `tests/test_hebrew_leakage.py` passing unchanged). Single database each (no surveillance/team-status side-stores — Stage 4c is explicit that no camera-state table or "active cameras" projection is in scope, so these agents keep only simple in-memory lists, matching `ReferenceAgent`'s/`FriendlyForcesAgent`'s existing style, not `StandbySquadSurveillanceAgent`'s heavier SQLite-store style).
- **Found and reported, not changed:** `docs/profile_spec.md`'s `EVENT_TYPES` is a plain `list[str]` — there is no structural per-event-type description field in the profile contract, and `history/event_pipeline.py`'s extraction prompt passes the bare type-name list to the model with no accompanying description text at all (confirmed by reading `_prompt()`). Stage 4's instruction to "write each description so it states what belongs in it and what does not" has no existing wiring to attach to; inventing one would be new core surface area Stage 4 doesn't explicitly authorize (unlike Stage 3's explicit migration/field-vocabulary authorization). Wrote the required descriptive text as inline comments directly above each `EVENT_TYPES` entry in both profile files instead — present in the codebase, readable, but not fed into the live classification prompt. Flagged here for the user to decide whether wiring a real per-type description channel into extraction is wanted as a follow-up (out of this task's authorized scope).
- Stage 4c (`entities` for multi-camera reports): confirmed `entities` is already stored as a `list`/JSON column and already populated by extraction; the generic (no domain terms) instruction "list every identifier the report refers to, e.g. equipment or unit identifiers" was already added to the core extraction prompt as part of the Stage 1 change (same prompt text, same file, same commit boundary — documented there too). No camera-state table or active-cameras projection was added, per the explicit out-of-scope list.
- Added `tests/test_operational_profiles.py`: both profiles load/validate through `profiles.loader.load_profile`; each exposes exactly its declared event types/areas/agents/protocols; no FIRE protocol or tool name exists in the SEC profile and vice versa (excepting the deliberately shared `attendance_update`/`record_availability`/`roster_agent`); every side-effecting tool's `idempotent` flag matches Stage 4's specification exactly; `request_mutual_aid` refuses an unrecognized resource name and records nothing for the refusal; the two profiles run side by side as two deployments with separate databases and ports (direct `SQLitePersistence` isolation check, mirroring `tests/test_integration_profile_isolation.py`'s own pattern).
- Focused verification: 16 new operational-profile tests, plus `tests/test_architecture.py`, `tests/test_hebrew_leakage.py`, `tests/test_messages.py`, `tests/test_agent_registry.py`, `tests/test_agent_runtime.py`, `tests/test_agent_permission_enforcement.py`, `tests/test_legacy_imports.py` (115 tests) all passed.

## 2026-09-24 - docs/bar_improves.md Stage 5: simulation as a separate deployment

- Checked the actual question Stage 5 poses before building anything: can a profile module import declared content (`AGENTS`/`PROTOCOLS`/`EVENT_TYPES`/`AREAS`/`EVENT_TYPE_REQUIRED_FIELDS`) from another profile module and still load/validate cleanly through `profiles.loader.load_profile`, including its own independent `profile_file_hash`? Verified directly (real `load_profile` calls against both new profiles and their sim counterparts): **yes** — `profiles.loader.load_profile` has no mechanism that couples two profile modules together or forbids one importing plain Python names from another; `hash_profile_file(module_path)` (used for `GET /SYSTEM`'s `profile_file_changed`) reads only the named module's own file bytes, so `profiles/response_team_sim.py` and `profiles/response_team.py` load with two different, genuinely independent hashes even though their declared content is identical. Proceeded with the "if yes" branch.
- Added `profiles/response_team_sim.py` (port 8917, `DB_PATH` under `data/response_team_sim/`, `BOT_TOKEN_ENV = "RESPONSE_TEAM_SIM_BOT_TOKEN"`, `PROFILE_NAME = "Response Team (Simulation)"`) and `profiles/fire_station_sim.py` (port 8918, `data/fire_station_sim/`, `FIRE_STATION_SIM_BOT_TOKEN`, `"Fire and Rescue Station (Simulation)"`) — each imports `AGENTS`/`PROTOCOLS`/`EVENT_TYPES`/`AREAS`/`EVENT_TYPE_REQUIRED_FIELDS` and the remaining starting-value/optimization attributes unchanged from its live counterpart and overrides only the four deployment-specific names above.
- Confirmed, by reading (not changing) `bot/app.py`/`bot/background_services.py`: the bot process's `SingleInstanceLock` is already keyed on `f"{loaded_profile.db_path}.bot.lock"` — since a simulation deployment has its own, distinct `DB_PATH`, its lock file is automatically distinct too. Live and simulation bots for the same organization can already run concurrently with no code change needed; this was a read-and-confirm item, not a fix.
- Documented in `docs/operator_guide.md` (new "Running a simulation deployment" section, ahead of "Writing a profile from scratch"): starting a simulation deployment; provisioning its users, including its own `bot-service` identity, with `cli/user_admin` run against the simulation profile module specifically; that simulation data can never reach the live database (distinct `DB_PATH`, no shared table or connection); that participants use the simulation bot (a distinct, real Telegram bot behind the `*_SIM_BOT_TOKEN` credential).
- Extended `tests/test_operational_profiles.py`: each sim profile loads/validates with the same declared agent/protocol/event-type/area content as its live counterpart but a different `db_path`/`api_port`/`profile_file_hash`; a live deployment and a simulation deployment share no users, no events (`fetch_events_range`), no precedent candidates (`fetch_events_by_type_area_window` — the live store's own lookback window never surfaces the simulation's event even though it shares classification/area), and no notifications (`fetch_notifications_since`, written as a side effect of setting an event's outcome).
- Focused verification: 19 operational-profile tests (full file) passed.

## 2026-09-24 - docs/bar_improves.md Stage 6: acceptance scenarios as offline regression tests

- `tests/api_fakes.py::build_context` builds a real `ApiContext` but always against its own fixed, generic fake protocols/registries — there is no existing helper that wires a *real* profile's declared agents/protocols/event-type-registry/area-registry into a real `ApiContext`. Rather than extend that shared fixture (used by dozens of other test files, and a needless risk to widen for a one-file need), added a small, file-local helper (`_operational_ctx`, `tests/test_operational_scenarios.py`) that calls `profiles.loader.load_profile` for the real `profiles.response_team`/`profiles.fire_station` module, then swaps `ctx.deps`'s `registry`/`protocol_set`/`event_type_registry`/`area_registry` via `dataclasses.replace` — the exact same technique `tests/test_api_messages.py`'s own `_ctx_with_protocol_flags` already uses to customize a `build_context` result without touching the shared fixture.
- **Found and worked around a real architectural constraint, not a bug:** `orchestrator.reasoning.formulate_tasks`'s JSON formulation path requires *exactly* one step per participating agent (`len(planned_steps) != len(descriptors)` and a repeated `agent_name` both raise `OrchestrationParseError`) — this codebase has no mechanism for two formulated steps against the same agent in one protocol run. Scenario 4 ("two cameras in one message... one log_camera_fault result per camera") therefore cannot be modeled as two persisted steps; a real model handling the one formulated task would call `log_camera_fault` twice *within* that single step (exactly what `SurveillanceFaultAgent`'s system prompt instructs), which an offline, model-free test has no way to observe. Verified instead what the pipeline actually guarantees offline: extraction captures both camera identifiers into `entities`, and the one formulated task names both. Documented in the test itself, not silently narrowed.
- **A second, related limitation, documented and worked around locally, not fixed:** the shared `_mock_crewai` fixture every other integration test in this codebase already uses returns one fixed canned string ("status nominal, no anomalies") from every fake `crewai.Agent.kickoff` call, regardless of task or tools — so no existing integration test can observe a *specific* tool's actual return text as a step's persisted result, only that some result exists. Built a small, file-local (not shared) fake `crewai.Agent` for this one file whose `kickoff(text)` deterministically echoes `f"recorded: {text}"` — since this file fully controls the formulated task text (a scripted Main Agent response), this is enough to assert e.g. "the step result says a dispatch request was recorded" truthfully, without claiming anything about real model behavior.
- Reports go through the real API (`POST /Event`, `POST /Approve`, `GET /Notifications` — the same `build_app(ctx).test_client()` pattern `tests/test_api_holds.py`/`tests/test_api_messages.py` already use). The one exception is scenario 1's follow-up reply: `POST /Event` accepts no `conversation_id` (confirmed by reading `docs/api_spec.md`'s own `/Event` request shape and `tests/test_api_holds.py::_make_clarification_hold`'s own comment making the identical choice for the identical reason), so there is no HTTP-level conversational-reply path for an `/Event`-originated hold; resolved the same way `_make_clarification_hold` already does — update the persisted fields directly and call `orchestrator.flows.resume_after_event_data` — rather than switching ingestion to `/Msg` for one scenario and taking on intent-classification scripting for no other reason.
- Added `tests/test_operational_scenarios.py` with all 9 scenarios from Stage 6 (SEC 1-6, including both the approve and reject variants of 6; FIRE 7, including both the viewer-held and commander-bypass variants; robustness 8 and 9) plus one extra (6's reject variant was written as its own test rather than folded into 6, since Stage 6 lists it as "a second variant"). Every assertion is on system-guaranteed state (classification, area, entities, selected protocol, hold status/reason, notification target, step count/agent names, outcome) — never on model wording, per Stage 6's own explicit rule.
- Focused verification: 11 new scenario tests passed. Full `tests/test_orchestrator_reasoning.py`, `tests/test_orchestrator_flows.py`, `tests/test_orchestrator_holds.py`, `tests/test_orchestrator_insights.py`, `tests/test_orchestrator_selection.py`, `tests/test_question_answering.py` (201 tests) passed alongside it, confirming the new file-local fake crewai agent didn't leak into or affect any other test module.

## 2026-09-24 - docs/bar_improves.md Stage 7: the tool-result rule in documentation and judgment

- Added a new section to `docs/agent_authoring.md` ("A tool result proves only the tool's own effect") stating the rule plainly, with the exact right/wrong phrasing pair Stage 6/agent-authoring already implicitly followed (`"...dispatch request recorded for 'west_gate'"` vs. `"...force dispatched/arrived"`), and pointing at `orchestrator.reasoning.judge_success`'s judgment prompt as the other place the same rule now lives in the running system, not just documentation.
- Added the identical rule to `orchestrator/reasoning.py`'s `_build_judgment_prompt` — the exact module the prompt text already lived in (confirmed by reading it; not moved). The addition is one clause inserted into the existing instruction sentence, so the response-format contract (`VERDICT:`/`REASONING:` two-line reply) is completely unchanged.
- Added `test_judgment_prompt_states_a_tool_result_proves_only_its_own_effect` and `test_judge_success_never_produces_a_stored_step_result_claiming_arrival` to `tests/test_orchestrator_judgment.py` — the first asserts on the built prompt's own text (this codebase's own words, not a model's), the second asserts on the tool's own persisted step result (never on the judge's verdict/reasoning, which is scripted and therefore not a meaningful thing to assert model wording about). Added `test_every_side_effecting_sec_tool_states_only_its_own_recorded_effect`/`test_every_side_effecting_fire_tool_states_only_its_own_recorded_effect` to `tests/test_operational_profiles.py`, calling every Stage-4 side-effecting tool method directly and checking its return text contains "recorded" and none of a fixed list of unobserved-outcome words ("arrived", "dispatched successfully", "extinguished", "rescued", ...) — confirming Stage 4's own tools already follow the rule Stage 7 makes explicit.
- Focused verification: 2 new judgment tests + 2 new operational-profile tests passed; full `tests/test_orchestrator_judgment.py`, `tests/test_orchestrator_reasoning.py`, `tests/test_operational_profiles.py`, `tests/test_operational_scenarios.py` passed together.

## 2026-09-24 - Follow-up to Stage 4 deviation 3: EVENT_TYPE_DESCRIPTIONS, a structural home for event-type descriptions

- Added an optional profile attribute `EVENT_TYPE_DESCRIPTIONS` (`dict[str, str]`, event type name -> English description), following exactly `EVENT_TYPE_REQUIRED_FIELDS`'s existing pattern end to end: `profiles.contracts.LoadedProfile` gained `event_type_descriptions` (defaults to empty — every existing profile/fixture unaffected); `profiles.loader.load_profile` reads it via `getattr(profile_module, "EVENT_TYPE_DESCRIPTIONS", {})`; `profiles.loader.validate_profile` rejects a key naming an event type the profile doesn't declare in `EVENT_TYPES`, an empty/whitespace-only description string, and (mirroring the identical existing rule for `unclassified`'s required fields) a description declared for the built-in `unclassified` type.
- `profiles.contracts.EventTypeRegistry` gained a `descriptions` field and a `description_for(event_type)` method (mirroring `required_fields_for`), exposed the same way through `profiles.loader.build_event_type_registry`. `description_for` returns `None` for `unclassified` unconditionally, the same "fixed here, never profile-declared" shape `required_fields_for` already uses for that type's required fields.
- `history/event_pipeline.py`'s `_prompt()` gained an optional `event_type_descriptions` parameter (threaded from `extract_event` via `getattr(event_type_registry, "descriptions", None)`): each event type name in the classification list that has a declared description gets it appended, in one `"Event type descriptions: <type>: <text> | ..."` clause; a type with no declared description is unaffected. Verified directly (not just via the regression test) that when no descriptions are given the prompt is byte-for-byte identical to before this change — the empty-string default contributes nothing to the f-string concatenation.
- Moved the inline `# comment` descriptions that Stage 4 had left as the only place this text lived (see the prior entry's own "Found and reported, not changed" note) into `EVENT_TYPE_DESCRIPTIONS` in both `profiles/response_team.py` and `profiles/fire_station.py`, reusing the existing comment text verbatim (that text was itself already adapted from `docs/bar_improves.md` §4a/§4b when Stage 4 was built) rather than re-deriving fresh wording. `EVENT_TYPES` in both files is now a plain list of names again.
- `profiles/response_team_sim.py` / `profiles/fire_station_sim.py` do **not** automatically inherit a new attribute merely by importing from the live profile module — their `from profiles.response_team import (...)` / `from profiles.fire_station import (...)` statements name every reused attribute explicitly (not `import *`), so `EVENT_TYPE_DESCRIPTIONS` needed an explicit added line in each; without it `getattr(profile_module, "EVENT_TYPE_DESCRIPTIONS", {})` would have silently fallen back to empty for both simulation deployments. Added the line to both (confirmed directly: the simulation profile's `loaded.event_type_descriptions` now equals the live profile's).
- Added `test_event_type_descriptions_for_an_undeclared_event_type_is_rejected`, `test_event_type_descriptions_with_an_empty_string_is_rejected`, `test_event_type_descriptions_with_a_whitespace_only_string_is_rejected`, `test_declaring_a_description_for_unclassified_is_rejected`, `test_valid_event_type_descriptions_declaration_reports_no_failures` to `tests/test_profile_loading.py` (mirroring the existing `EVENT_TYPE_REQUIRED_FIELDS` validation tests' exact shape; extended the shared `_loaded()` test helper with an `event_type_descriptions` parameter). Added `test_extraction_prompt_includes_each_declared_event_type_description`, `test_extraction_prompt_omits_description_text_for_a_type_that_has_none`, `test_extraction_prompt_is_unchanged_when_the_profile_declares_no_event_type_descriptions` (the explicit regression check), and `test_extraction_prompt_carries_a_real_response_team_profiles_declared_descriptions` (loads the real `profiles.response_team` profile and confirms its own declared description text reaches the prompt) to `tests/test_history_event_pipeline.py`.
- Focused verification: `tests/test_history_event_pipeline.py`, `tests/test_profile_loading.py`, `tests/test_operational_profiles.py`, `tests/test_operational_scenarios.py` together — 130 passed. **Full-suite verification was not completed for this change**: mid-verification, a broader combined run (these four files plus `tests/test_architecture.py`, `tests/test_file_catalog.py`, `tests/test_hebrew_leakage.py`, `tests/test_messages.py`, `tests/test_orchestrator_flows.py`, `tests/test_orchestrator_reasoning.py`, `tests/test_integration_profile_loading.py`, `tests/test_integration_profile_isolation.py`, `tests/test_integration_end_to_end_flow.py` — 108 passed, 4 failed) was interrupted by an explicit instruction to stop running tests, before a full, isolated run of the whole suite could be completed or the 4 failures (all in `tests/test_integration_end_to_end_flow.py`, all asserting on `model_io` JSON log records inside a captured-stdout string — logging plumbing this change never touches) could be confirmed as pre-existing/order-dependent versus caused by this change. Flagged explicitly in the handback rather than assumed either way.

## 2026-09-24 - docs/responce_improve.md: Response Team unification (one profile, one database, SEC_001 as a live event)

- Implemented all 8 stages of the locked design. `profiles/response_team.py` was rewritten in full: profile constants (`AREAS`, `CAMERAS`, `DRONES`, `DRONES_WAREHOUSE`, `FORCE_BASES`, the 9x9 ETA matrix + `eta_seconds()`), three profile-only agent classes (`ResponseTeamRosterAgent(TeamStatusAgent)`, `ResponseTeamSurveillanceAgent(SurveillanceAgent)`, `NeighboringForcesAgent(Agent)` — new), the 7-protocol/7-event-type set the spec's table specifies (all `approval_flag=False`), an `OPERATIONAL_SEED` provisioning hook, and the ported SEC_001 `SIMULATION_USERS`/`GROUPS`/`ROSTERS`/`SIMULATIONS`. New `persistence/response_team_store.py` (`ResponseTeamRosterStore`, `ResponseTeamSurveillanceStore`, `NeighboringForceStore`) opens the *same* `DB_PATH` as the profile's core `events`/`users` tables and runs its own `CREATE TABLE IF NOT EXISTS` DDL — never touching the shared `persistence/schema.py`. Deleted `profiles/standby_squad.py`, `profiles/response_team_sim.py`, and the now-fully-superseded `agents/response_team_agents.py` (its `SecurityOpsAgent`/`SurveillanceFaultAgent` had no callers left once the protocol/agent set was rebuilt).
- **What SEC_001 turned out to be:** the `SIMULATIONS` (`sec001_phase1/2/3`) previously declared in the deleted `profiles/standby_squad.py`, itself migrated from `fixtures/admin_scenarios/'כיתת כוננת - חלק 1/2/3.json'` — confirmed by reading that content directly (Hebrew personas, camera-fault/security-incident narrative). Ported unchanged in substance into `profiles/response_team.py`; only the message-catalog key prefix (`standby_squad.` → `response_team.`, 178 keys in each of `messages/he.py`/`messages/en.py`) and the SEC_001 camera-number narrative text changed.
- **SEC_001 camera-ID mapping, resolved from a real inconsistency:** the existing Hebrew and English SEC_001 text used *different* narrative camera numbers for the same story beats (Hebrew: "מצלמה 01/02/03"; English: "Camera 03/08", with no "04" anywhere). Cross-referencing both against the spec's own `old-number -> id/area` table (`camera 03 -> CAM-01 east_fence`, `camera 04 -> CAM-02 east_fence`, `camera 08 -> CAM-03 south_corner`) showed the English numbering, not the Hebrew, already matched the spec by narrative role (first-mentioned/taken-down fence camera, the second one "next to it", the interference one at the south corner) — so both languages were rewritten to reference `CAM-01`/`CAM-02`/`CAM-03` directly, resolving the pre-existing inconsistency in the same pass rather than propagating it.
- **ETA ticker, implemented as a documented deviation:** rather than a new background polling loop + new API endpoint mirroring `POST /TeamStatus/AttendanceCheck` (the spec's literal suggestion), `NeighboringForceStore._advance()` computes the `en_route -> arrived` transition lazily on every `dispatch()`/`list_dispatches()` call — the same "compute derived state as of now" idiom `persistence/team_status_store.py`'s own `availability_snapshot(as_of)` already uses. Observably identical (a dispatch shows `arrived` once its ETA has elapsed, discoverable via `list_neighboring_force_dispatches`), with no new bot-loop/transport/endpoint surface area.
- **Provisioning, implemented as a documented deviation:** rather than editing `api/app.py`'s `build_context` call site directly (which would require it to import a specific profile module, violating `tests/test_architecture.py`'s package-boundary rule), `profiles/simulation_provisioning.py`'s `ensure_simulation_entities` now resolves an optional `OPERATIONAL_SEED` callable off the loaded profile module (via `importlib.import_module(loaded_profile.module_path)`, the same dynamic-import technique `profiles.loader._import_profile_module` already uses) and calls it after the existing user/group/roster provisioning. `profiles/response_team.py` declares `OPERATIONAL_SEED = _ensure_operational_seed_data`, which inserts cameras/drones if missing and opens today's attendance cycle if the roster is already approved and none is open yet. Guarded with `getattr(..., None)`/`try/except ImportError` since several existing tests call `ensure_simulation_entities` against a hand-built `SimpleNamespace`, not a real `LoadedProfile`.
- **Unplanned but necessary ripple-effect fixes**, discovered only by grepping the whole repo for `standby_squad` after deleting it: `config/server_control.py`'s `SELECTABLE_PROFILE_STEMS`/`load_selected_profile` default (the admin dashboard's two-profile switcher) pointed at `standby_squad`; retargeted to `response_team`. Also updated: `README.md`'s quickstart commands/port, `.env.example`'s token/port documentation, and `--profile`/module-path defaults or example text in `cli/group_admin.py`, `tools/terminal_client_viewer.py`, `tools/terminal_client_commander.py`, `bot/app.py`, `bot/simulator_app.py`, `api/app.py`, `my_fake_bot_test.py`, `tests/sanity_check_real_model_call.py` (the last two are standalone, not-pytest-collected scripts, fixed for correctness, not test-suite necessity). `BOT_TOKEN_ENV` was deliberately *not* changed to the shared `"BOT_TOKEN"` firefighting.py/(deleted) standby_squad.py used — `profiles/response_team.py` keeps its own pre-existing `"RESPONSE_TEAM_BOT_TOKEN"`, a smaller-blast-radius deviation from that shared-token idiom, noted here rather than silently changed.
- **Deviation:** `profiles/response_team.py`'s new `ResponseTeamSurveillanceAgent` also exposes the shared `SurveillanceAgent`'s inherited `return_drone_to_base`, `update_camera_observation`, and `get_camera_feeds` tools alongside the new `recall_drone`/`update_camera_status` (no protocol references the inherited three; harmless surface area, not suppressed).
- **Criticality levels** for the 7 new protocols are not specified by the spec's own protocol table (only `approval_flag`/`commander_only`/`requires_confirmation` are pinned at `False` for all seven); chosen consistent with the equivalent protocols in the deleted `profiles/standby_squad.py`/`profiles/firefighting.py` (`report_security_incident`/`dispatch_neighboring_force` HIGH, `update_camera_status` MEDIUM, the rest LOW).
- Retargeted tests the spec named (`tests/test_operational_profiles.py`, `tests/test_operational_scenarios.py`, file-catalog exactness) plus tests the spec didn't name but that broke once `standby_squad`/`response_team_sim` were deleted: `tests/test_server_control.py`, `tests/test_api_admin.py` (one assertion), `tests/test_profile_simulations.py` (the SEC_001-migration and roster-provisioning tests), `tests/test_profile_loading.py` (one test). Deleted `tests/test_standby_squad_role_and_security.py` outright rather than retargeting line-by-line — its entire subject (Hebrew-localized tool text, the standby-squad-specific commander/viewer Telegram keyboards, the four-tool `FriendlyForcesAgent`-based dispatch flow) has no equivalent in the new design, since `profiles/response_team.py`'s own tool text stays English throughout (the architecture rule's own wording) and the new `NeighboringForcesAgent` has one dispatch tool, not four.
- `.github/workflows/ci.yml`: removed the deleted test file's line; added none (no new test *files* were created, only existing ones edited). **Found, not caused by this task:** the workflow's own "Validate offline test-module coverage" step (inline Python, not a pytest test) was already failing before this task started — 14 `tests/test_*.py` files on disk (`test_api_groups.py`, `test_api_simulations.py`, `test_api_situational_picture.py`, `test_bot_groups.py`, `test_bot_simulator_app.py`, `test_bot_simulator_transport.py`, `test_cli_group_admin.py`, `test_group_routing.py`, `test_integration_profile_simulations.py`, `test_run_stack.py`, `test_situational_picture.py`, `test_unsafe_system.py`, plus `test_server_control.py`/`test_profile_simulations.py` which this task did edit) were never declared in any CI mission, pre-dating this task and unrelated to `standby_squad`/`response_team`/`SEC_001` in content. Not fixed (out of this task's scope); all 12 not otherwise touched here were run and pass regardless (see verification below).
- Deleted the demo camera-catalog auto-seed pattern for this profile only: `ResponseTeamSurveillanceStore` (unlike the shared `persistence/surveillance_store.py`) seeds nothing itself; cameras/drones come only from `OPERATIONAL_SEED`'s `CAMERAS`/`DRONES` tuples, per the spec's "seed and area names come from the profile, not hardcoded standby sectors."
- Focused verification, run file by file to match this repo's actual CI structure (10 separate `pytest` invocations, one per Mission, not one combined collection — a single `pytest tests/` run was tried and aborted after ~15 minutes with zero output as unrepresentative of how CI itself verifies green): every one of this repository's 88 `tests/test_*.py` files was run, with the CI-matching environment (`TEST_CORE_MODEL_PROVIDER=openai`, `TEST_CORE_MODEL_NAME=test-core-model`, `TEST_CORE_MODEL_API_KEY_ENV=CORE_TEST_KEY`, `TEST_CORE_TEST_KEY=test-key`, `TEST_SUB_MODEL_PROVIDER=openai`, `TEST_SUB_MODEL_NAME=test-sub-model`, `TEST_SUB_MODEL_API_KEY_ENV=SUB_TEST_KEY`, `TEST_SUB_TEST_KEY=test-key`, `DEEP_DEBUG=false`, `DEBUG_VERBOSE_LOGGING=false`, `OBSERVABILITY_MODE=log`, `PYTHONDONTWRITEBYTECODE=1`), and with this shell's own pre-set `ADMIN_USERNAME`/`ADMIN_PASSWORD`/`ADMIN_SESSION_SECRET`/`LOG_CONSOLE_JSON` explicitly unset first (their leaking presence caused 8 false failures across `tests/test_integration_end_to_end_flow.py`, `tests/test_integration_log_sink.py`, `tests/test_integration_user_administration.py` on the first pass — confirmed as pure environment leakage, not a regression, by re-running with a clean environment). **Result: 1520 passed, 0 failed, across all 88 files** (Mission 1: 177; Mission 2: 113; Mission 3: 127; Missions 4+5: 110; Mission 6: 231; Mission 7: 262; Mission 8: 254; Mission 9: 46; Mission 10 + `test_server_control.py` + `test_profile_simulations.py`: 76; the 12 CI-undeclared files above: 124).
- Confirmed via `git status`/`git diff` that `profiles/fire_station.py`, `profiles/firefighting.py`, and `profiles/fire_station_sim.py` were not modified, and that no git write command (`add`/`commit`/`push`/etc.) was run at any point.
