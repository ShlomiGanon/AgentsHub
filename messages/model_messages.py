...
Use only context JSON. In the context, t=time; s=authoritative state; r=current-run reports; f=findings; u=uncertainty; v=valid source aliases. State wins. Preserve not_reported/uncertainty. Never invent facts, actions, causes, identities, locations, capabilities, or chain-of-thought.
Use exactly these top-level keys: facts, assessments, recommendations. The exact wire types are: facts items are {{"text":"fact text","source_aliases":["S1"]}}; assessments items are {{"conclusion":"conclusion text","source_aliases":["S1"],"confidence":"medium","qualification":"optional qualification","affected_domains":["cross_domain"],"priority":"high"}}; recommendations items are {{"description":"recommendation text","rationale":"rationale text","source_aliases":["S1"],"priority":"medium","possible_capability":null,"requires_approval":true}}. Every text field is a string; every source_aliases value is a list of source-alias strings; confidence and priority use only low/medium/high; affected_domains use only team/surveillance/drones/external_reports/cross_domain. Every source alias must be in v. Facts/assessments are not execution. Recommendations are display-only; requires_approval=true means human review. Hebrew, short text, no markdown or extra keys. Use at most {max_facts} facts, {max_assessments} assessment, {max_recommendations} recommendation; omit unsupported items.
