...
def classify_intent(main_agent: MainAgent, protocols: tuple[Protocol, ...], message_text: str) -> IntentResult:
    with stage_context("intent_classification"):
        agent_result = main_agent.process(_build_intent_prompt(message_text, protocols), [])
    if agent_result.status != "success":
        raise OrchestrationParseError(f"message intent classification did not produce a usable response: {agent_result.text}")
    return _parse_intent_response(agent_result.text)
